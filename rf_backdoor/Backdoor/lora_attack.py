...
def backdoor_train(bad_encoder, train_loader, criterion, optimizer, device, epochs=10, show_interval=50):
    """
...
def train_downstream(epochs, downstream, backbone, train_loader, val_loader, optimizer, criterion, device, show_interval=10):
    for epoch in range(epochs):
        ...
        if (epoch + 1) % show_interval == 0:
            re_num_params = sum(p.numel() for p in backbone.parameters() if p.requires_grad)
            print('Number of trainable parameters: %d' % re_num_params)
...
if __name__ == '__main__':
    ...
    substitute_data = np.load('/aul/homes/tzhao010/INFOCOM25/backdoor/Benign_Models/LoRa/sub_lora_data_norm.npy')
    substitute_data = substitute_data.transpose(0, 2, 1)
    print('Number of substitute:', substitute_data.shape)

    # use how many data to re-train the encoder
    bad_encoder_train_epochs = 300
    data_rate = 0.2
    poison_rate = 0.1
    n = 7
    trigger_size = 48
    ...
    # model_name = '/aul/homes/tzhao010/INFOCOM25/backdoor/Benign_Models/Bert/lora_bert_model.pth'
    model_name = '/aul/homes/tzhao010/INFOCOM25/backdoor/Benign_Models/LoRa/lora_trans_simclr.pth'
    # model_name = '/aul/homes/tzhao010/INFOCOM25/backdoor/Benign_Models/Backdoor/lora_res_simclr_retrain.pth'
    print('Model:', model_name)
    # model_name = '/aul/homes/tzhao010/INFOCOM25/backdoor/Benign_Models/Backdoor/lora_trans_tstcc_retrain.pth'
    # backbone = Adaptive_ResNet18()
    backbone = Transformer(n_channels=2, len_sw=256, n_classes=10, dim=256, depth=4, heads=4, mlp_dim=128, dropout=0.1, backbone=True)
    backbone.load_state_dict(torch.load(model_name, map_location=device))
    backbone.to(device)
    backbone.eval()
    # bad_encoder = Adaptive_ResNet18()
    bad_encoder = Transformer(n_channels=2, len_sw=256, n_classes=10, dim=256, depth=4, heads=4, mlp_dim=128, dropout=0.1, backbone=True)
    ...
    # torch.save(bad_encoder.state_dict(), '../BadEncoder/bad_lora_res_simclr.pth')
    bad_encoder.load_state_dict(torch.load('../BadEncoder/bad_lora_trans_simclr.pth', map_location=device))

    for p in bad_encoder.parameters():
        p.requires_grad = False

    # check any block is frozen
    for name, param in bad_encoder.named_parameters():
        if param.requires_grad:
            print(name)

    ############################ Load data ############################
    our_data = np.load('/aul/homes/tzhao010/LoRa_RFFI/dataset/Train/test_lora_data.npy')
    our_label = np.load('/aul/homes/tzhao010/LoRa_RFFI/dataset/Train/test_lora_labels.npy')
    our_label = np.squeeze(our_label)
    our_data = our_data.transpose(0, 2, 1)

    our_train_loader, our_val_loader, our_test_loader = train_val_test_split(our_data, our_label)
    our_downstream = Downstream(bad_encoder, n_classes=10).to(device)
    our_optimizer = optim.Adam(our_downstream.parameters(), lr=1e-3)
    our_criterion = nn.CrossEntropyLoss()
    train_downstream(200, our_downstream, bad_encoder, our_train_loader, our_val_loader, our_optimizer, our_criterion, device, 50)
    print('Test -----Benign Accuracy on bad encoder:')
    test_downstream(our_downstream, our_test_loader, device)

    _, our_test_data, _, our_test_labels = train_test_split(our_data, our_label, test_size=0.2, random_state=42)
    print('Number of samples in test data:', len(our_test_data))
    our_predicted_list, our_acc_list = test_poisoned_ds(trigger, our_test_data, our_test_labels, our_downstream, device)
